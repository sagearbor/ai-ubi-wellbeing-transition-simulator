...
export function operativeCoverage(draft: Partial<PolicyDraft>, sourceText?: string): import('./types').OperativeCoverage {
  ...
  const validDisposition = (d: NonNullable<PolicyDraft['clauseDispositions']>[number] | undefined): boolean => !!d && typeof d.reason === 'string' && !!d.reason.trim()
    && ['linked','unresolved','outside-model','not-operative'].includes(d.status)
    && (d.provisionIds === undefined || (Array.isArray(d.provisionIds) && d.provisionIds.every(id => draft.provisions?.some(p => p.id === id))))
    && (d.status !== 'linked' || (Array.isArray(d.provisionIds) && d.provisionIds.length > 0));
  const uniqueDisposition = (id: string) => {
    const matches = dispositions.filter(d => d?.clauseId === id);
    return matches.length === 1 && validDisposition(matches[0]) ? matches[0] : undefined;
  };
  const unresolved = src.detail.filter(c => {
    const d = uniqueDisposition(c.id);
    return !d || d.status === 'unresolved' || (d.status === 'linked' && d.provisionIds!.some(id => draft.provisions?.some(p => p.id === id && p.status === 'unresolved')));
  }).map(c => c.id);
  ...
  const accounted = src.detail.filter(c => uniqueDisposition(c.id)).length;
  const invalidBookkeeping = dispositions.some(d => !validDisposition(d) || !src.detail.some(c => c.id === d.clauseId) || !uniqueDisposition(d.clauseId));
  return { clauses: src.clauses, accounted, unresolved, outsideModel, text: src.status === 'source-unavailable' || src.status === 'source-mismatch' ? 'Operative coverage unknown — source unavailable or mismatched' : `${accounted} of ${src.clauses} source clauses have explicit operative dispositions; ${unresolved.length} unresolved, ${outsideModel.length} outside model — ${invalidBookkeeping ? 'invalid bookkeeping; validation must be resolved' : accounted < src.clauses || unresolved.length || outsideModel.length ? 'partial scenario; unsupported effects are not zero effects' : 'bookkeeping complete, semantic completeness requires identified review'}` };
}
