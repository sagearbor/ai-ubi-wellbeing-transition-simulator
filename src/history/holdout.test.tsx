...
describe('held-out presentation',()=>{
 ...
 it('shows the untuned primary failure by default with units and both views',()=>{const html=renderToStaticMarkup(<HistoryExperience/>);expect(html).toContain('aria-pressed="true">Held-out test');expect(html).toContain('Historical reconstruction');expect(html).toContain('worse than persistence');for(const v of [artifact.scores.outcomes.ladder.overall.model.mae,artifact.scores.outcomes.ladder.overall.persistence.mae,artifact.scores.outcomes.ladder.overall.modelMinusPersistenceMae])expect(html).toContain(holdoutFormat(v,3));expect(html).toContain('0–10');expect(html).toContain('0–100');expect(html).toContain('Does the model beat predicting no change?');expect(html.indexOf('100 origin countries')).toBeLessThan(html.indexOf('<figure'));expect(html.indexOf('worse than persistence')).toBeLessThan(html.indexOf('<figure'));expect(html.indexOf('Aggregate held-out comparison')).toBeGreaterThan(html.indexOf('</figure>'));expect(html).toContain('691 / 700');expect(html).toContain('681 / 700');expect(html).toContain('not test the conditional-world default');});
});
