...
describe('held-out presentation',()=>{
 ...
 it('shows the untuned primary failure by default with units and both views',()=>{const html=renderToStaticMarkup(<HistoryExperience/>);expect(html).toContain('aria-pressed="true">Held-out test');expect(html).toContain('Historical reconstruction');expect(html).toContain('worse than persistence');for(const v of [artifact.scores.outcomes.ladder.overall.model.mae,artifact.scores.outcomes.ladder.overall.persistence.mae,artifact.scores.outcomes.ladder.overall.modelMinusPersistenceMae])expect(html).toContain(holdoutFormat(v));expect(html).toContain('0–10');expect(html).toContain('0–100');expect(html).toContain('691 / 700');expect(html).toContain('681 / 700');expect(html).toContain('not test the conditional-world default');});
});
