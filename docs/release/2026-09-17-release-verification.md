...
- Clean committed production build **`510867fc16a4f929fd3d017dd1988d1634d09626`** passed the release checker on `http://127.0.0.1:4195`: matching embedded commit, `dirty: false`, HTML entry and all **15 emitted JavaScript/CSS assets**. The displayed build badge agreed. Keyboard ArrowRight advanced the History inspection year from 2020 to 2021.
- On that exact production candidate, a real extraction initially encountered a connection failure; retry returned the provider's website-address restriction. The UI preserved the source and offered manual drafting; that fallback retained both clauses as unresolved and did not assert review. No credential restrictions were changed. **Successful live extraction on the final candidate remains unverified** until it runs on an authorized deployed origin.
- Opened the actual public allocation AI bundle in the committed candidate: **all 180 stored values reproduced, largest deviation 0**. The bundle retained AI authorship, the unsupported risk clause and experimental imported-model status. This verifies candidate replay, not candidate extraction.
- [GitHub CI for the implementation commit](https://github.com/sagearbor/ai-ubi-wellbeing-transition-simulator/actions/runs/35282237171) passed in 3m31s. [Release PR #35](https://github.com/sagearbor/ai-ubi-wellbeing-transition-simulator/pull/35) carries these changes and the final documentation record. GitHub merge and cloud deployment remain distinct; this record does not claim a public rollout.
