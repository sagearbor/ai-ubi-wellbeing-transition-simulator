...
Reviewed 2026-09-17. This is a literature and source review with an independent audit of saved companion results; it introduces no new model comparison. The companion machine-readable inventory is `data/evaluation/annual-literature-20260917/sources.json`. Dataset access and public release status were checked on this date. No restricted Gallup dataset was acquired.
...
| Fixed-origin conditional reconstruction | Fit before the cutoff; supply later observed predictors; hide all later targets | Transport of an association given realized inputs |
| Sequential one-year conditional reconstruction | Freeze coefficients; supply realized predictors and the previously observed outcome at each new origin; hide the current target | One-step conditional performance with annual outcome updating |
...
**Public annual target access and collection were subsequently confirmed.** On the official [Finland country dashboard](https://data.worldhappiness.report/country/FIN), the root agent selected **Annual score** and read the visible 2025 tooltip: **7.811**, versus **7.764** for the three-year score. The completed study target contains **159 country-years in eight predetermined countries, 2005/06–2025**: Brazil, China, Germany, United Kingdom, India, Japan, United States and South Africa. These are transcriptions of public chart marks, not a bulk original annual data release.

The final extraction used rendered SVG circle coordinates and visible linear-axis tick positions after selecting annual mode, corrected the axis half-pixel stroke offset, and rounded to three decimals. Stored annual tooltip spot-checks cover the latest year in all eight countries; the collector also reports checking US 2024. An attempted repeated-tooltip collection produced stale overlay readings and was discarded. Raw chart attributes, method and spot-check labels are preserved in the wellbeing study target provenance. Independent arithmetic in this review reproduces all **159** stored values exactly at three decimals and matches all **eight** stored tooltip checks; this validates the stored coordinate conversion, not every live webpage value independently. Missing country-years were left absent. No hidden application state, undocumented API or gated download was used. This limited route does not provide the annual social-predictor panel, and public display does not establish unrestricted redistribution rights.
...
## Independent audit of the annual wellbeing demonstration

Reviewed `scripts/evaluation/annual-wellbeing/{model.py,prepare.py,run.py,test_model.py}`, original registration `2076670`, scope amendment `437dd16`, frozen commit `9a49e76562acc0b1229e32a7f88f18c7ce62f79b`, source provenance and saved outputs. The scoring receipt records one completed run on 2026-09-17 at 04:58:14 UTC. **No P1 or P2 implementation defect was found in the audited scope.** The scorer was not rerun, no model was refitted, and no candidate or tuning change was introduced by this review. Machine-readable findings and exact exclusions are in `data/evaluation/annual-literature-20260917/wellbeing-audit.json`.

- All **15** hashes passed: eight frozen files, four outputs and three referenced World Bank source files. The copied target has SHA-256 `0ad3698a5b63cb8761c0cacd831415e839dac9d8a6ccaace4c4a7008c156c3be`.
- All **159** target rows and **477** joined feature values matched their sources and registered transforms. Each country has 11 complete initial training rows. The source gaps, including China 2022, remain absent.
- Read-only arithmetic reproduced all **12 pooled and 96 country metric rows**, including 2,592 numeric/null fields, plus 12 country-equal mean errors. All **130** included country-year-mode sets have the same six methods, yielding **780** predictions. Every displayed path agrees with the saved row predictions.
- Expected masks, origins, cutoffs, training-row counts and missingness agree. Conditional reconstruction has **61 rows in 2017–2024**; rolling prediction has **69 rows in 2017–2025**. Missing 2025 total life expectancy prevents conditional reconstruction that year but does not block rolling predictions using earlier drivers. China's missing 2022 target also removes its 2023 and 2024 predictions because consecutive outcome lags are required.
- All predictions are finite and within 0–10; none hits a clipping boundary. Scaling and country offsets use training data only. Rolling drivers are projected exclusively from the two previous dated rows. Target-year wellbeing is attached after prediction and is not a predictor.
- The original registered formulas, fitting cutoffs and shrinkage settings are unchanged in the final implementation. Before scoring, amendments restricted acquisition to the already named eight countries, documented chart transcription and added descriptive direction comparisons. The cohort restriction is disclosed; this is not a globally representative trial or an untouched outcome period.

Three interpretation limits should remain visible beside the graphs. **First, conditional coefficients are frozen at 2016, but the displayed predictions repeatedly receive observed previous-year wellbeing.** This is sequential one-year conditional reconstruction, not a free-running path from 2016. **Second, the conditional and rolling pooled results cover different years and masks.** Their difference cannot isolate the benefit of observing target-year drivers. **Third, the only candidate with lower rolling pooled MAE is half-change shrinkage: 0.22279 versus persistence 0.22551, a reduction of about 0.00272 ladder points or 1.206%.** It wins on 39 of 69 paired errors and loses on 30; the nominal two-sided sign-test p-value is 0.336 before accounting for serial/country dependence. It improves country MAE in three of eight countries. This result does not establish a reliable winner or close annual forecasts. All four candidates have worse conditional MAE than persistence.

Observed directions are balanced here: 31 rises and 30 falls in the conditional sample, 35 rises and 34 falls in the rolling sample. The rolling change model and its shrinkage share 40 correct directions in 69 cases (58.0%); shrinkage changes magnitude, not sign. Persistence always predicts zero change and all these observed changes are nonzero, so its 0% direction score and direction-comparison p-values are not evidence of a useful magnitude improvement. Level MAE and same-anchor one-year change MAE are algebraically identical, not independent successes.

The audit establishes internal consistency of this retrospective reduced-information demonstration. It does not remove survey noise, transcription uncertainty, revised macroeconomic data, unknown historical publication dates, omitted annual social measurements, or uncertainty about generalization beyond the eight chosen countries.
