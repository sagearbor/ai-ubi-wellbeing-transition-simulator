...
For graphs that render directly in GitHub without downloading, open `docs/design/reviews/2026-09-16-visual-results.md`. Its five PNG figures are generated by `render_github_figures.py` with Matplotlib 3.10.8, using only the saved path and metric artifacts. `figures/2026-09-16-forecast-paths/provenance.json` records the input hash and renderer version. No experiment is run when the figures are rendered.
