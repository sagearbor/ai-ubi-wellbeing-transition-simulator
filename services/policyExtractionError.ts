...
export function policyExtractionError(error: unknown): string {
  ...
  if (name === 'PolicyRateLimitError' || /RESOURCE_EXHAUSTED|\b429\b/.test(payload)) return 'The AI extraction limit was reached. Try again later or use a manual draft; your pasted text is preserved.';
  ...
  if (/SAFETY|RECITATION|PROHIBITED_CONTENT|blockedReason|refus/i.test(payload)) return 'The AI provider declined this extraction. Review the source text or use a manual draft; your pasted text is preserved.';
