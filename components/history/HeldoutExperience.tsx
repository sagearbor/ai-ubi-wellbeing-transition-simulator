...
export default function HeldoutExperience() {
  ...
  return <section className="history-experience holdout-experience" aria-labelledby={`${id}-title`}>
    <header><h1 id={`${id}-title`}>Does the model beat holding 2018 constant?</h1>
      <p className="history-boundary"><strong>The first, untuned wellbeing result is worse than persistence.</strong> Across all observed test country-years, model mean absolute error is <strong>{fmt(primary.model.mae)}</strong> ladder points; persistence is <strong>{fmt(primary.persistence.mae)}</strong>. The model misses by <strong>{fmt(primary.modelMinusPersistenceMae)} more ladder points</strong> on average.</p>
      ...
      <p>This is a retrospective holdout using revised data and model-form choices made after the test period. It is neither a blind independent forecast nor an archived 2018 forecast.</p>
    </header>
    <section aria-label="Aggregate held-out comparison"><h2>The whole origin cohort comes first</h2><p>{artifact.origin.countries.length} countries fixed at the origin; {artifact.origin.excluded.length} excluded before test outcomes were read. Scores weight observed country-years equally, with independent outcome masks shared by model and persistence.</p>
      <div className="history-table-scroll"><table><caption>Mean absolute error (MAE), all test years pooled. Lower is better.</caption><thead><tr><th scope="col">Outcome</th><th scope="col">Model MAE</th><th scope="col">Persistence MAE</th><th scope="col">Model minus persistence</th><th scope="col">Observed / eligible</th></tr></thead><tbody>{([['Wellbeing ladder points (0–10)',primary],['GDP growth percentage points',secondary]] as const).map(([label,s])=><tr key={label}><th scope="row">{label}</th><td>{fmt(s.model.mae)}</td><td>{fmt(s.persistence.mae)}</td><td>{fmt(s.modelMinusPersistenceMae)} ({s.modelMinusPersistenceMae===null?'not scored':s.modelMinusPersistenceMae>0?'worse':'lower error'})</td><td>{s.observed} / {s.expected}; {s.unscored} missing</td></tr>)}</tbody></table></div>
    ...
    <details><summary>Annual values and errors: {name}</summary><DataTable rows={rows}/></details>
    <details><summary>All country-year scores, both outcomes</summary><DataTable rows={artifact.scores.rows}/></details>
