...
const PolicyResults: React.FC<PolicyResultsProps> = ({ model, entries, active, year, onYear, modelStatus = 'curated' }) => {
  ...
  return (
    <div className="space-y-3">
      ...
      {firstChanged !== null && <p role="status" className="rounded-lg bg-slate-100 dark:bg-slate-800 p-3 text-sm">For the displayed entity, the shown differences are zero in {timeLabel(model.time, years[t])}; another year differs. This does not mean the policy has no real-world effects. <button type="button" className="underline font-semibold" onClick={() => onYear(firstChanged)}>Show first changed {unitName.toLowerCase()}: {timeLabel(model.time, firstChanged)}</button></p>}
